...
@Component("saleCommand")
public class SaleCommand implements IndustrikaCommand {
	...
	public HashMap<String, Object> execute(Map<String, String[]> parameters) {
		...
		results.put("form", "/sales/sale.jsp");
		if (parameters.get("returnType") != null && ((String[])parameters.get("returnType"))[0].equalsIgnoreCase("jsp")){
			...
			Sale dto = makeDtoForWeb(parameters);
			...
			if (action != null && action.equalsIgnoreCase("search")){
				try{
					...
					Vector<Sale> lista = new Vector<Sale>(dao.find(dto, order));
					if (lista != null && lista.size() == 1){
						Sale firstOrder = lista.get(0);
					...
					if (lista != null && lista.size() >0){
						...
						results.put("message", SalesMessages.getMessage("sale.search.notfound"));
					}
			...
			if (action != null && action.equalsIgnoreCase("remove")){
				try{
						...
						dao.delete(dto);
						Sale buyOrder= new Sale();
						buyOrder=tranferValues(dto,buyOrder);
						dto=buyOrder;
						results.put("message", SalesMessages.getMessage("sale.remove.persintence"));
			...
			if (action != null && action.equalsIgnoreCase("update")){
				try{
					dao.update(dto);
					results.put("message", SalesMessages.getMessage("sale.update.persintence"));
				}catch(Exception ex){
					results.put("error", ex.getMessage());
				}
			}
	...
	private Sale makeDtoForWeb(Map<String, String[]> parameters){
		Sale dto = new Sale();
	...
	private Sale tranferValues(Sale dto, Sale tranfer){
