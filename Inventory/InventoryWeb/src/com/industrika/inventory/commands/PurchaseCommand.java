...
@Component("purchaseCommand")
public class PurchaseCommand implements IndustrikaCommand {
		...
		if (parameters.get("returnType") != null && ((String[])parameters.get("returnType"))[0].equalsIgnoreCase("jsp")){
			...
			if (action != null && action.equalsIgnoreCase("search")){
				try{
					...
					Vector<Purchase> lista = new Vector<Purchase>(buyDao.find(dto, order));
					if (lista != null && lista.size() == 1){
						Purchase firstOrder = lista.get(0);
							...
							if (actualRow.getItem().getTaxes() != null && actualRow.getItem().getTaxes().size()>0){
								for (Tax tx : actualRow.getItem().getTaxes()){
									if (tx.isPercentage() && actualRow.getItem().getCost() != null){
										taxes += actualRow.getItem().getCost().doubleValue()*(tx.getTaxValue()/100);
									} else {
										taxes += tx.getTaxValue();
									}
								}
							}
							simpleRow.setItemId(""+actualRow.getItem().getIdItem()+"_"+actualRow.getItem().getCost()+"_"+taxes);
					...
					if (lista != null && lista.size() >0){
						...
						results.put("message", InventoryMessages.getMessage("purchase.search.notfound"));
					}
			...
			if (action != null && action.equalsIgnoreCase("remove")){
				try{
						...
						buyDao.delete(dto);
						Purchase buyOrder= new Purchase();
						buyOrder=tranferValues(dto,buyOrder);
						dto=buyOrder;
						results.put("message", InventoryMessages.getMessage("purchase.remove.persintence"));
			...
			if (action != null && action.equalsIgnoreCase("update")){
				try{
					buyDao.update(dto);
					results.put("message", InventoryMessages.getMessage("purchase.update.persintence"));
				}catch(Exception ex){
					results.put("error", ex.getMessage());
				}
			}
	...
	private Purchase makeDtoForWeb(Map<String, String[]> parameters){
				...
				if(((String[])parameters.get("rowsAsJson"))!=null){
					...
					if(rows!=null && !rows.isEmpty()){
						...
						for(SimpleDocumentRow row: myRows){
							...
							docRow.setAmount(docRow.getQuantity()*(docRow.getItem().getCost() != null ? docRow.getItem().getCost() : 0));
		...
		if (parameters.get("providersAccount") == null || ((String[])parameters.get("providersAccount"))[0].trim() ==""){
			try {
				...
				if (providers != null && providers.size() >= 1){
					...
					Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
					String json = gson.toJson(providerArray);
					dto.setProvidersAccount(json);
				}
			} catch (Exception e) {
				e.getMessage();
			}
		} else{
			dto.setProvidersAccount(((String[])parameters.get("providersAccount"))[0].trim());
		}
					...
					if (itemVal.getTaxes() != null && itemVal.getTaxes().size()>0){
						for (Tax tx : itemVal.getTaxes()){
							if (tx.isPercentage() && itemVal.getCost() != null){
								taxes += itemVal.getCost().doubleValue()*(tx.getTaxValue()/100);
							} else {
								taxes += tx.getTaxValue();
							}
						}
					}			
					itemsSt.append(itemVal.getIdItem()+"_"+itemVal.getCost()+"_"+taxes+":"+itemVal.getName() +" - "+price);
					i++;
					if(i<=items.size()){
						itemsSt.append(";");
					}
	...
	private Purchase tranferValues(Purchase dto, Purchase tranfer){
